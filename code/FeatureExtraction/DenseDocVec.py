...
def getLSAVectors(tok_tweets):
  ...
  logger.info("Applying SVD to the processed matrix of shape {}".format(X_tfidf.shape))
  
  denseLSAvecs = svd.fit_transform(X_tfidf)
  
  logger.info("Created LSA matrix of shape : {}".format(denseLSAvecs.shape))
...
def getDoc2VecVectors(tok_tweets):
  ...
  logger.info("Created paragraph2vec matrix of shape : {}".format(d2v.shape))
