...
def getLSAVectors(tok_tweets, k = 300, ngram_range = (1,3)):
  """
  ...
  :param k: number of latent dimension
  :type k: int
  :param ngram_range: ngram range to take into consideration
  :type ngram_range: tuple
  :rtype: numpy.ndarray
  """
    
  tfidf_vec = TfidfVectorizer(preprocessor = ' '.join, tokenizer = str.split, ngram_range = ngram_range, analyzer = 'word')
  
  X_tfidf = tfidf_vec.fit_transform(tok_tweets)
    
  svd = TruncatedSVD(n_components = k, random_state = 7)
  
  logger.info("\n\nApplying SVD to the processed matrix of shape {0} with ngram range = {1} and k = {2}".format(X_tfidf.shape,ngram_range,k))
...
def getDoc2VecVectors(tok_tweets,dm = 1, window = 3, dm_mean = 1, negative = 50, size = 300):
  """
  ...
  :param tok_tweets: tokenized tweets
  :type tok_tweets: list of lists
  :param dm: 1 is PV-DM, 0 is PV-DBOW
  :type dm: int
  :parma window: window size
  :type window: int
  :param dm_mean: wheter to take mean or averaging vectors
  :type dm_mean: int
  :param negative: number of words sampled with negative sample
  :type negative: int
  :param size: size of the embeddings
  :type size: int
  ...
  model_pv_dm = doc2vec.Doc2Vec(dm = dm, window = window, size = size, hs = 0 , min_count = 1, dm_concat = 0,
                                dm_mean = dm_mean, workers = 1,negative = negative, seed = 7)
